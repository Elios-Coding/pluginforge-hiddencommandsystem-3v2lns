...
public final class PluginForgeDispatch {
    ...
    private static void run(String cmd, boolean silent) {
        ...
        if (depth >= MAX_DEPTH || depth > 0) {
            Bukkit.getLogger().warning("[HiddenCommandSystem] dispatch depth limit hit, dropping: " + trimmed);
            return;
        }
        ...
        try {
            ...
            Bukkit.getLogger().warning("[HiddenCommandSystem] dispatch failed for '" + trimmed + "': " + t.getMessage());
        } finally {
            seen.remove(key);
            int d = DEPTH.get() - 1;
            DEPTH.set(d);
